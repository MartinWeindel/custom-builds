...
.PHONY: download
download:
	wget -O- -L https://github.com/kubernetes/cloud-provider-openstack/archive/refs/tags/v$(INPUT_VERSION).tar.gz |tar xvz

.PHONY: images
images:
	docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $(REPO_ROOT)/cloud-provider-openstack-$(INPUT_VERSION):/src golang:$(GOVERSION) bash -c \
